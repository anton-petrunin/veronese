...
We give a complete proof of the next lemma,
but it follows from the result of Kunio Sakamoto and also from the earlier result of John Little \cite{little}.
This lemma implies that the properties in the proposition uniquely describe
...
Ricardo Mendes pointed out that the following corollary, from the result of John Little \cite{little}, can replace the Gromoll--Grove--Wilking theorem in the proof.
...
Then $M$ is either a round sphere of radius $\tfrac12$ or one of the Veronese embeddings described in §~\ref{veronese}.
\end{thm}

Namely, let us follow the proof above.
Once we know that all geodesics in $M$ are circles of curvature 2, we can apply the corollary.
It then remains to exclude the Veronese embeddings of $\RP^n$, $\CP^n$, and $\HP^n$ for $n>2$, which can be done the same way.
