...
Let \( M \subset \RR^d \) be a closed smooth \( n \)-dimensional submanifold.
Assume that \( d \) is large and \( M \) lies in an \( r \)-ball.
\textit{What can be said about the normal curvatures of \( M \)?}
...
where \( H \) denotes the mean curvature vector.
...
\label{thm:strict}
\textit{Let \( M \) be a closed smooth \( n \)-dimensional submanifold contained in a closed \( r \)-ball in \( \RR^d \).
...
By assumption, the curvature of \( \gamma \) in \( \RR^d \) is less than 2.
Applying Schur's bow lemma, we obtain \( |x - y| > 1 \).
...
Therefore,
\[
\langle y - x, \gamma'(t_0) \rangle > \int\limits_0^{\frac{\pi}{2}} \cos(2 \cdot |t - t_0|) \cdot dt \geq 0.
\]
...
Therefore, Schur's bow lemma applies to $\hat{\gamma}_{\mathsc{u}}$, and hence $|y - z| \z> 1$.
...
It follows that the exponential map $\exp_x$ defines a local diffeomorphism $V_x \to W_x$.
...
We will need the following claim, which is a special case of John Little's theorem \cite{little}.
It also follows from the more general results of Kunio Sakamoto from the same paper.
...
in this case, \( n \ge 2 \).
As before, let \( \mathbb{B}^d \) denote the \( r \)-ball in \( \RR^d \), where we assume \( r = \tfrac{1}{\sqrt{3}} \);
...
My original proof was based on the diameter-rigidity theorem, which was proven by Detlef Gromoll, Karsten Grove, and Burkhard Wilking \cite{gromoll-grove,wilking}.
