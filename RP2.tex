...
We can assume that $r=\tfrac1{\sqrt{3}}$; that is, $r$ is the curcumradius of equilateral triangle with side 1.
...
Recall that a compact rank-one symmetric spaces is isometric to rescaled copy of one the following spaces:
...
Moreover, the images of these embeddings lie in a sphere of radius $r_n=\sqrt{n/(2\cdot n+2)}$ (for $\OP^2$, we assume that $n=2$). 
\end{thm}
...
Note that $r_n$ is the circumradius of regular $n$-simplex with edge length~$1$.
...
Recall that Veronese embedding maps $\RP^n$, $\CP^n$, and $\HP^n$ into balls of radius $r_n= \sqrt{n/(2\cdot n+2)}$, which is circumradius of regular $n$-simplex with edge length~$1$.
...
If, in addition, $M$ lies in the boundary of the $r$-ball, then by the Gauss formula \cite[Lemma 5]{petrunin2024}, its curvature is strictly quarter-pinched,
